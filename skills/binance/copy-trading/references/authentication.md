...
* `User-Agent`: binance-copy-trading/1.0.0 (Skill)
...
curl -s -X GET "${BASE_URL}/sapi/v1/lending/copy-trading/target-asset/list?${QUERY}&signature=${SIGNATURE}" \
...
curl -s -X POST "${BASE_URL}/sapi/v1/lending/copy-trading/plan/add?${QUERY}&signature=${SIGNATURE}" \
  -H "X-MBX-APIKEY: ${API_KEY}" \
  -H "User-Agent: binance-copy-trading/1.0.0 (Skill)"
```
...
curl -s -X POST "${BASE_URL}/sapi/v1/lending/copy-trading/one-off?${QUERY}&signature=${SIGNATURE}" \
  -H "X-MBX-APIKEY: ${API_KEY}" \
  -H "User-Agent: binance-copy-trading/1.0.0 (Skill)"
```
